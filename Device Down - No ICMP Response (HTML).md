# Device Down - No ICMP Response (HTML)

## Alert Title
```text
@if($alert->state == 0)✅ DEVICE UP:@else🚨 DEVICE DOWN:@endif {{ strtoupper(explode('.', $alert->hostname)[0]) }} - No ICMP Response
```
  ...
  <tbody>
    ...
    @endif
        ...
        <td colspan=2 style="padding: 10px; text-transform: uppercase;" class="header">
          @if ($alert->state == 1)
            <span class="ALERT">DEVICE DOWN - NO ICMP RESPONSE</span>
          @elseif ($alert->state == 2)
    ...
    <tr>
        <td colspan=3 class="critical-box">
          <strong>⚠️ Network Connectivity Lost:</strong> Device is not responding to ICMP ping requests. The device may be powered off, network connectivity may be interrupted, or ICMP may be blocked by a firewall.
        </td>
    </tr>
    @else
    <tr>
        <td colspan=3 class="recovered-box">
          <strong>✅ Network Connectivity Restored:</strong> Device is now responding to ICMP ping requests.
        </td>
    </tr>
    @endif
    <tr>
    ...
    <tr>
        <td colspan=3 style="padding: 10px;">
          <ul style="margin: 5px 0; padding-left: 20px;">
            <li><strong>Verify device power status</strong> - Check if device is powered on and booting properly</li>
            <li><strong>Check physical connections</strong> - Verify network cables are connected and link lights are active</li>
            <li><strong>Test network path</strong> - Ping device from different network segments to isolate issues</li>
            <li><strong>Check switch/router ports</strong> - Verify upstream network equipment is functioning</li>
            <li><strong>Review firewall rules</strong> - Ensure ICMP traffic is not being blocked</li>
            <li><strong>Check for scheduled maintenance</strong> - Verify if this is a planned outage</li>
            <li><strong>Physical inspection</strong> - If possible, visually inspect device for hardware failures</li>
          </ul>
        </td>
    </tr>
    @endif
    ...
    <tr>
        ...
        <td>{{ $alert->name ?? 'Device Down - No ICMP Response' }}</td>
    </tr>
    <tr>
